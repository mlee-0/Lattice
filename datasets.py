        ...
        for line in lines:
            ...
            if d > 0:
                ...
                for j, node_numbers_ in enumerate(node_numbers_rotated):
                    ...
                    # # Ensure the first node is within bounds. If both are within bounds, make the smaller node number the first node.
                    # is_node_1_rotated_oob = any(
                    #     coordinate >= INPUT_SHAPE[_] - (STRUTS_PER_NODE[_]-1)
                    #     for _, coordinate in zip(range(3), (x1, y1, z1))
                    # )
                    # is_node_2_rotated_oob = any(
                    #     coordinate >= INPUT_SHAPE[_] - (STRUTS_PER_NODE[_]-1)
                    #     for _, coordinate in zip(range(3), (x2, y2, z2))
                    # )
                    # if is_node_1_rotated_oob and not is_node_2_rotated_oob:
                    #     node_1_rotated, node_2_rotated = node_2_rotated, node_1_rotated
                    # elif not is_node_1_rotated_oob and is_node_2_rotated_oob:
                    #     pass
                    # elif not is_node_1_rotated_oob and not is_node_2_rotated_oob:
                    #     node_1_rotated, node_2_rotated = sorted([node_1_rotated, node_2_rotated])

                    row = node_1_rotated - 1
                    column = (strut_numbers[(node_1_rotated, node_2_rotated)] - 1) % w
                    # # Array of node numbers surrounding this node.
                    # x, y, z = np.argwhere(node_numbers_ == node_1)[0, :]
                    # neighborhood = node_numbers[x:x+STRUTS_PER_NODE[0], y:y+STRUTS_PER_NODE[1], z:z+STRUTS_PER_NODE[2]]

                    # x_, y_, z_ = np.argwhere(node_numbers_ == node_1)[0, :]
                    # row = node_numbers[x_, y_, z_] - 1
                    # column = np.argwhere(np.sort(neighborhood.flatten()) == node_2)[0, 0]
